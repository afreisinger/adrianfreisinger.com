---
name: Adrián Freisinger
profession: Electronic Engineer
education: 'Facultad de Ingeniería de la Universidad de Buenos Aires, UBA'
work: 'Agencia de Recaudación y Control Aduanero, ARCA'
location: 'Buenos Aires, Argentina'
email: 'info@adrianfreisinger.com'
cover: ./me.jpg
skills:
  - Backend Development (Python, Go, FastAPI, PostgreSQL, Redis)
  - Frontend Development (React, Gatsby, Styled Components)
  - DevOps (Docker, Kubernetes, Terraform, Ansible, Git, GitHub Actions)
  - Electronics (Arduino, ESP32, Raspberry Pi)
  - AI (LangChain, LlamaIndex, Ollama, OpenAI)
  - Cloud (AWS, OpenStack, Netlify)
---

Hello! My name is Adrián Freisinger and I enjoy solving real-world problems and bring
them down to code. My interest in coding started during my freshman year at college.
At [ARCA](https://www.arca.gob.ar/), I am the part of Infrastructure and Development Platform Department team. We are developing engineering solutions focused on improving user experience and streamlining problem resolution through well-designed digital systems

Here are a few technologies I've been working with recently:
